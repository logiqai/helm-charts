---
description: >-
  This page describes the LOGIQ.AI deployment on Kubernetes cluster using HELM 3
  charts.
---
...
LOGIQ.AI K8S components are made available as helm charts. Instructions below assume you are using HELM 3. Please read and agree to the [EULA](https://docs.logiq.ai/eula/eula) before proceeding.

### 1.1 Add LOGIQ.AI helm repository
...
logiq-repo/logiq    v3.0.7             v3.5.8.7b      LOGIQ.AI Observability Data Fabric for Kubernetes
```

### 1.2 Create namespace where LOGIQ.AI will be deployed
...
This will create a namespace **`logiq`** where we will deploy the LOGIQ.AI Log Insights stack.
...
[LOGIQ.AI Sample Values.yaml files](https://docs.logiq.ai/logiq-server/k8s-quickstart-guide#1-3-prepare-your-values-yaml-file)
...
```
Please refer [Section 3.10 ](k8s-quickstart-guide.md#3-10-sizing-your-LOGIQ.AI-cluster) for sizing your LOGIQ.AI cluster as specified  in these yaml files.
...
This will install LOGIQ.AI and expose the LOGIQ.AI services and UI on the ingress IP. Please refer [Section 3.4 ](k8s-quickstart-guide.md#3-4-changing-the-storage-class)for details about storage class. Service ports are described in the [Port details section](https://docs.logiq.ai/logiq-server/quickstart-guide#ports). You should now be able to go to `http://ingress-ip/`
...
![Logiq Insights Login UI ](https://github.com/logiqai/docs/raw/master/.gitbook/assets/Screen Shot 2022-03-19 at 8.11.14 AM.png)

LOGIQ.AI server provides Ingest, log tailing, data indexing, query and search capabilities.  
Besides the web based UI, LOGIQ.AI also offers [logiqctl, LOGIQ.AI CLI](https://docs.logiq.ai/logiq-cli) for accessing the above features.
...
> You should now be able to login to LOGIQ.AI UI at your domain using `https://logiq.my-domain.com` that you set in the ingress after you have updated your DNS server to point to the Ingress controller service IP
>
...
> The `logiq.my-domain.com` also fronts all the LOGIQ.AI service ports as described in the [port details section](quickstart-guide.md#ports).

| HELM Option | Description | Defaults |
| :--- | :--- | :--- |
| `global.domain` | DNS domain where the LOGIQ.AI service will be running. This is required for HTTPS | No default |
...
> Please note that cloud providers may charge data transfer costs between regions. It is important that the LOGIQ.AI cluster be deployed in the same region where the S3 bucket is hosted
...
#### 3.2.2 Deploy the LOGIQ.AI helm in gateway mode
...
>
> If the bucket already exists, LOGIQ.AI will use it. Check to make sure the access and secret key work with it. Additionally, provide a valid amazon service endpoint for s3 else the config defaults to [https://s3.us-east-1.amazonaws.com](https://s3.us-east-1.amazonaws.com)
...
### 3.3 Install LOGIQ.AI server certificates and Client CA `[OPTIONAL]`

LOGIQ.AI supports TLS for all ingest. We also enable non-TLS ports by default. It is however recommended that non-TLS ports not be used unless running in a secure VPC or cluster. The certificates can be provided to the cluster using K8S secrets. Replace the template sections below with your Base64 encoded secret files.

> If you skip this step, the LOGIQ.AI server automatically generates a ca and a pair of client and server certificates for you to use. you can get them from the ingest server pods under the folder `/flash/certs`
...
Save the secret file e.g. `logiq-certs.yaml`. Proceed to install the secret in the same namespace where you want to deploy LOGIQ.AI

The secret can now be passed into the LOGIQ.AI deployment
...
If you are planning on using a specific storage class for your volumes, you can customize it for the LOGIQ.AI deployment. By default, LOGIQ.AI uses the `standard` storage class
...
To use external AWS RDS Postgres database for your LOGIQ.AI deployment, execute the following command.
...
| :--- | :--- | :--- |
| `global.chart.postgres` | Deploy Postgres which is needed for LOGIQ.AI metadata. Set this to false if an external Postgres cluster is being used | true |
...
```bash
helm install logiq --namespace logiq \
--set global.environment.admin_name="LOGIQ.AI Administrator" \
...
| HELM Option | Description | Default |
| :--- | :--- | :--- |
| `global.environment.admin_name` | LOGIQ.AI Administrator name | flash-admin@foo.com |
| `global.environment.admin_password` | LOGIQ.AI Administrator password | flash-password |
| `global.environment.admin_email` | LOGIQ.AI Administrator e-mail | flash-admin@foo.com |

### 3.8 Using external Redis instance

To use external Redis for your LOGIQ.AI deployment, execute the following command.

> NOTE: At this time LOGIQ.AI only supports connecting to a Redis cluster in a local VPC without authentication
...
When deploying LOGIQ.AI, configure the cluster id to monitor your own LOGIQ.AI deployment. For details about the `cluster_id` refer to section [Managing multiple K8S clusters](agentless.md#managing-multiple-k-8-s-clusters-in-a-single-logiq-instance)
...
| HELM Option | Description | Default |
| :--- | :--- | :--- |
| global.environment.cluster\_id | Cluster Id being used for the K8S cluster running LOGIQ.AI. See Section on [Managing multiple K8S](agentless.md#managing-multiple-k-8-s-clusters-in-a-single-logiq-instance) clusters for more details. | LOGIQ.AI |

### 3.10 Sizing your LOGIQ.AI cluster

When deploying LOGIQ.AI, size your infrastructure to provide appropriate vcpu and memory requirements. We recommened the following minimum size for small. medium and large cluster specification from [Section 1.3 ](k8s-quickstart-guide.md#1-3-prepare-your-values-YAML-file) values yaml files.

| LOGIQ.AI Cluster | vCPU| Memory | NodeCount |
| :--- | :--- | :--- | :--- |
| small | 12| 32 gb | 3 |
| medium  | 20| 56 gb | 5 |
| large  | 32| 88 gb | 8 |
...
For e.g. if you are running on bare-metal and want an external LB to front LOGIQ.AI, configure all services as `NodePort`
...
The LOGIQ.AI stack deployment can be optimized using node labels and node selectors to place various components of the stack optimally
...
The LOGIQ.AI stack includes Thanos as part of the deployment as an optional component for larger deployments. To enable Thanos, follow the steps below
